...
export async function open(input) {
  try {
    ...
    return `<@${interaction.member.user.id}> has opened a tournament in this channel. Use "/register" to join!${announceDeck}${announceSwaps} Rounds are ${t_defaults['elim_style']}.`;
  } catch (error) {
    console.log(error);
  ...
  try {
    ...
    switch (elim_style.toLowerCase()) {
      case 'swiss': {
        ...
        standings_text += `Round ${round} standings:\n`;
        var placement = 1;
      ...
      case 'single elimintation': {
        ...
        for (let i = 0; i < players_fetch['results'].length; i++) {
          ...
          if (ongoing_tournaments_fetch['results'][0]['decklist_pub'] == 'true') {
            ...
            standings_text += `${name}${format_l}<@${player_id}>${format_r}`;
          }
        }
      }
      default:
...
export async function process_register_modals(input) {
  try {
    ...
    if (components_data['modal_decklist']) {
      ...
      if (!deck_link.startsWith('https://www.moxfield.com/decks/') && !deck_link.startsWith('https://archidekt.com/decks/') && !deck_link.startsWith('https://tappedout.net/mtg-decks/')) {
        return 'Error: Decklink must be a moxfield, archidekt, or tappedout url.';
      }
    } else {
      var deck_link = '';
    }
...
export async function check_registered(input) {
  try {
    ...
    var player_data_fetch = await env.DB.prepare('SELECT player_id, deck_name, deck_link, name FROM players WHERE tournament_id = ? ORDER BY m_score').bind(tournament_id).run();
    ...
    var player_list = `Users registered for the tournament in the ${interaction.channel.name} channel of the ${interaction.guild.name} server:`;
    for (let i = 0; i < player_data_fetch['results'].length; i++) {
